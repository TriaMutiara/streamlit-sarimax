...
if unggahFile is not None:
    ...
    df.drop(['Tanggal', 'Jam', 'Hari'], axis=1, inplace=True) # Hapus kolom asli
    ...
    if st.button("Proses"):
        ...
        # Pengaturan peramalan 4 hari ke depan
        langkah_peramalan = 4
        hasil_peramalan = []

        # Buat indeks peramalan untuk 4 hari ke depan dengan jam yang sesuai
        last_date = df.index[-1]
        
        # Dapatkan jam-jam unik dari data untuk peramalan
        unique_hours = sorted(df.index.hour.unique())
        
        # Logika perbaikan: Buat forecast untuk 4 hari ke depan, satu forecast per hari
        forecast_dates = []
        
        # Gunakan jam yang berbeda untuk setiap hari
        for i in range(langkah_peramalan):
            # Tentukan hari ke berapa (1, 2, 3, 4 hari ke depan)
